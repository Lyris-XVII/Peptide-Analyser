...
def main() -> None:
    try:
        ...
        if user_choice == "1":  # Peptide chain analyser route.
            ...
            chain = peptide_check(raw_chain)  # Validates input.
            ...
            percentage_composition = calculate_percentage_composition(
                chain
            ...
            residue = residue_check(raw_residue)  # Validates residue input.
            table = residue_table(residue)  # Retrieves residue data.
            print(figlet.renderText("Results"))  # Displays result title.
...
def calculate_percentage_composition(chain: str) -> str:
    collection = {}  # Counts each residue.
    length = len(chain)  # Stores chain length.
    for x in chain:
        if x in collection:
            collection[x] += 1  # Increment if exists.
        else:
            collection[x] = 1  # Initialise if first occurrence.
    result = []  # Store formatted percentages.
    for key, value in sorted(collection.items()):
        percentage = (value / length) * 100  # Calculate percentage.
        result.append(f"{key} | {percentage:.2f}%")  # Format for display.
    return "\n".join(result)  # Combine for output.


def peptide_table(molecular_weight: float, hydrophobicity: float) -> list[list[str]]:
    table = [
...
def residue_table(residue: str) -> list[list[str | float]]:
    residue_info = amino_acid_data[residue]  # Get residue info.
    table = [
